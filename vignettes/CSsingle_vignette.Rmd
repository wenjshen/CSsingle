...
## Example 1: Cell type deconvolution in mixtures of two cell types of remarkedly different cell sizes: HEK and Jurkat cells
...
This step is to exclude cell types that are not interested from scRNA-seq data, remove sparse features and cells and trim bulk and scRNA-seq data to have the same features.
...
for (i in names(data.list)[-1]){
  ...
  gplots[[i]] <- ggplot(data=data, aes(x=Real,y=Estimated, label=cellType), common.legend = T)+
...
gscat.arrange = ggarrange(plotlist = gplots, ncol = 2, nrow = 2, labels = "")
...
## Example 2: Cell type deconvolution in in human Barrett’s esophagus tissue
...
gbxplot = ggplot(dat, aes(x=Tissue, y=est.prop, fill=Tissue)) +
  ...
  labs(x="", y="", title = "CSsingle") + theme_classic() + 
  scale_fill_brewer() + theme(legend.position="top", legend.title = element_text(size = 10), legend.text = element_text(size=10))
print(gbxplot)
```
