...
build_sigMat <- function(diffexpMat, ctDEGs, num.top, sort.by = "p.value", rm.dupli = TRUE, filepath = NULL){
  ...
  if (rm.dupli){
    dupli_ctDEGs <- list()
    for (i in 1:length(ctDEGs.cty)){
      markers.i <- ctDEGs.cty[[i]]
      markers.not.i <- unlist(ctDEGs.cty[-i])
      dupli_ctDEGs[[i]] <- markers.i[markers.i %in% markers.not.i]
    }
    dupli_ctDEGs <- unique(unlist(dupli_ctDEGs))
    topMarkers <- list()
    for (i in cty){
      topMarkers[[i]] <- setdiff(ctDEGs.cty[[i]], dupli_ctDEGs)
    }
