#' This function is to estimate the cell-type proportions from the individual bulk mixture \code{y}
#'
...
#' @export
#'
CSsingle_basis <- function(X, y, markers, cellSize = NULL, dampened = FALSE){
  log.X <- log(X + 1e-4)
  log.y <- log(y + 1e-4)
  intercepts <- c()
  present.ct.idx <- c()
  ...
  if(length(present.ct.idx) == 0){
    ...
    if (dampened == TRUE){
      ...
      for (h in 1:length(limit)){
        ...
        wnorm <- w$x/sum(w$x)
        correlation[h] <- round(cor(X %*% matrix(wnorm), y, method="spearman", use = "complete.obs"), 3)
      }
    ...
    while(change>0.01 & num.iter<1000){
      wnormNew <- CSsingle_basisJ(X, y, wnorm, markers, dampened)
